...
echo  "$($PSQL "SELECT name FROM teams AS losers LEFT JOIN games ON losers.team_id = games.opponent_id WHERE games.round= 'Eighth-Final' AND games.year=2014 UNION SELECT name FROM teams AS winners LEFT JOIN games ON winners.team_id = games.winner_id WHERE games.round= 'Eighth-Final' AND games.year=2014 ORDER BY name;")"
